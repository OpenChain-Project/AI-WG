# OpenChain AI Work Group

_TODO: description of the group_

## Goals

- _TODO: Goal 1_
- _TODO: Goal 2_
- _TODO: Goal 3_
- _etc_

## Non-Goals

- _TODO: Non-Goal 1_
- _TODO: Non-Goal 2_
- _TODO: Non-Goal 3_
- _etc_

## Deliverables

- [Artificial Intelligence System Bill of Materials](https://github.com/OpenChain-Project/AI-WG/blob/main/docs/Artificial%20Intelligence%20%20System%20Bill%20of%20Materials%20-%20Compliance%20Management%20Guide%20%20for%20the%20Supply%20Chain%20-%20RFC.docx)

## Work Group Members

_TODO: how to become a work group member_

- Matthew Crawford (co-chair)
- Dave Marr (co-chair)
- _TODO: other members_

## Discussion and Meetings

All meetings and discussions are open for anyone to participate in.

- [Calendar](https://zoom-lfx.platform.linuxfoundation.org/meetings/openchain?view=month&committee=ab046d9d-6a4b-489c-97e6-f750ff8a421d)
- [Mailing List](https://lists.openchainproject.org/g/ai)
- [Slack](https://slack.openchainproject.org)

